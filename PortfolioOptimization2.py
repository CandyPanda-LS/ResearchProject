...
class PortfolioOptimization(OptimizationApplication):
    ...
    def to_quadratic_program(self) -> QuadraticProgram:
        ...
        if self.bounds:
            ...
            y = [mdl.binary_var(name=f"y_{i}") for i in range(num_assets)]
        else:
        ...
        # Linking constraints and cardinality constraint
        large_value = 2 * self._budget
        # for i in range(num_assets):
        #     mdl.add_constraint(x[i] <= y[i] * large_value)
        #     mdl.add_constraint(y[i] <= x[i])
        # mdl.add_constraint(mdl.sum(y[i] for i in range(num_assets)) <= self._cardinality_limit)
